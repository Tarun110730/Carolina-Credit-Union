...
def batch_update_csv(deleted_names):
    """
    Update multiple people at once (for when you delete several people)
    """
    csv_path = 'Email Project with Data for Alumni Database - Job & Internship Acceptances (1).csv'
    ...
    with open(csv_path, 'r', newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            if row['Full Name'] in deleted_names:
                row['Status'] = 'Reachout Sent'
            rows.append(row)
    ...
    print(f"Updated {len(deleted_names)} people to 'Reachout Sent' status")
...
if __name__ == '__main__':
    ...
    # Example: Batch update multiple people
    # batch_update_csv(["Colin Sullivan", "Quinn Savell"]) 